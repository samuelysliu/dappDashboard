...
class model:
    def __init__(self):
        self.collection = db.table().diceTable()
    ...
    def getByAmount(self, *args):
        try:
            order = []
            for i in self.collection.find({"bet": args[0]["amount"]}):
                order.append(i)
            return order
        except:
            return "failed"
    ...
    def getByProbability(self, *args):
        try:
            order = []
            for i in self.collection.find({"probability": args[0]["probability"]}):
                order.append(i)
            return order
        except:
            return "failed"
